...
const HomeScreen = () => {
  ...
  const [selectedNav, setSelectedNav] = useState('Home'); // State cho mục đã chọn
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.bottomNav}>
        {['Home', 'Search', 'Favorites', 'Cart', 'Profile'].map((item, index) => (
          <TouchableOpacity
            key={index}
            style={[styles.navItem, selectedNav === item && styles.selectedNavItem]}
            onPress={() => setSelectedNav(item)} // Cập nhật trạng thái mục đã chọn
          >
            <Image
              source={require(`../assets/images/${item.toLowerCase()}_icon.png`)} // Đảm bảo các icon có tên phù hợp
...
const styles = StyleSheet.create({
  ...
  selectedNavItem: {
    borderBottomWidth: 2,
    borderBottomColor: '#000', // Màu khung dưới mục đã chọn
  },
  moreButton: {
    padding: 10,
    backgroundColor: '#000',
