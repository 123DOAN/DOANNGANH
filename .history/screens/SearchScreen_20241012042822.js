...
const SearchScreen = () => {
  ...
  const slideAnim = new Animated.Value(300); // Giá trị bắt đầu để trượt ra ngoài bên phải
  ...
  const toggleFilter = () => {
    if (filterVisible) {
      // Nếu bộ lọc đang hiển thị, trượt lại
      Animated.timing(slideAnim, {
        toValue: 300, // Trượt ra ngoài
        duration: 300,
        useNativeDriver: false,
      }).start(() => setFilterVisible(false));
    } else {
      // Nếu bộ lọc không hiển thị, trượt vào
      setFilterVisible(true);
      Animated.timing(slideAnim, {
        toValue: 0, // Trượt vào vị trí ban đầu
        duration: 300,
        useNativeDriver: false,
      }).start();
    }
      ...
      {filterVisible && (
        <Animated.View style={[styles.filterPanel, { transform: [{ translateX: slideAnim }] }]}>
          ...
          <TouchableOpacity onPress={toggleFilter}>
            <Text style={styles.closeFilter}>Đóng</Text>
          </TouchableOpacity>
        </Animated.View>
      )}
    </SafeAreaView>
